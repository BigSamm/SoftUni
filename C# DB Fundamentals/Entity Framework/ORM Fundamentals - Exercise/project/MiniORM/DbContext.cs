...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        private void MapCollection<TDbSet, TCollection>(DbSet<TDbSet> dbSets, PropertyInfo collectionProperty)
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            if (isManyToMany)
            {
                primaryKey = collectionType.GetProperties()
                    .First(pi => collectionType.GetProperty(pi
                        .GetCustomAttribute<ForeignKeyAttribute>().Name).PropertyType == entityType);
            }

            DbSet<TCollection> navigationDbSet = (DbSet<TCollection>)this.dbSetProperties[collectionType].GetValue(this);

            foreach (TDbSet entity in dbSets)
            {
        ...
        {
            string tableName = this.dbSetProperties[tableType].Name;
